	    ...
	    if( $order ){
	    	...
	    	if ( $order->has_status( 'failed' ) ) :
				...
				echo '<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed-actions">
					<a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" class="button pay">'.esc_html__( 'Pay', 'woocommerce' ).'</a>';
...
if (! function_exists('sc_cart_cross_sells')) {
	function sc_cart_cross_sells($attr) {
		...
		$output .= '<'.$title_tag.' class="title heading">'.( !empty($attr['heading']) ? $attr['heading'] : __( 'You may be interested in&hellip;', 'woocommerce' ) ).'</'.$title_tag.'>';
...
if (! function_exists('sc_banner_box')) {
	function sc_banner_box($attr) {
		...
		$elements = array(
			'subtitle' => !empty($subtitle) || !empty($_GET['visual']) || !empty($attr['vb']) ? '<'.$subtitle_tag.' '.( empty( $subtitle ) ? 'style="display: none;"' : '' ).' class="subtitle banner-item">'.$subtitle.'</'.$subtitle_tag.'>' : '',
			'title' => !empty($title) || !empty($_GET['visual']) || !empty($attr['vb']) ? '<'.$title_tag.' '.( empty( $title ) ? 'style="display: none;"' : '' ).' class="title banner-item">'.$title.'</'.$title_tag.'>' : '',
		...
		if( !empty( $attr['link_type'] ) && !empty($attr['popup_id']) ){
			...
			$wrapper_attr = 'data-mfnpopup="'.$attr['popup_id'].'"';
		}
		...
		$output .= '<div class="'.implode(' ', $classes).'">';
		    ...
		    $output .= '<div class="banner-image">';
		    	$output .= '<img class="scale-with-grid svg" src="'.( !empty($image) ? $image : get_theme_file_uri( '/muffin-options/svg/placeholders/image.svg' ) ).'" alt="">';
		    $output .= '</div>';

		    if( !empty( $attr['bb_badge'] ) || !empty($_GET['visual']) || !empty($attr['vb']) ) $output .= '<div '.( empty( $attr['bb_badge'] ) ? 'style="display: none;"' : '' ).' class="banner-badge banner-badge-'.( !empty($attr['bb_badge_pos']) ? $attr['bb_badge_pos'] : 'top-right' ).'">'.( !empty( $attr['bb_badge'] ) ? $attr['bb_badge'] : '').'</div>';
...
if (! function_exists('sc_product_upsells')) {
	function sc_product_upsells($attr, $product = false)
	{
		...
		$title_tag = !empty($attr['heading_tag']) ? $attr['heading_tag'] : 'h3';
		$output .= '<'.$title_tag.' class="title heading">'.__( 'You may also like&hellip;', 'woocommerce' ).'</'.$title_tag.'>';
		$output  .= '<ul class="products '.$ul_classes.'">';
...
if (! function_exists('sc_product_related')) {
	function sc_product_related($attr, $product = false)
	{
		...
		$title_tag = !empty($attr['heading_tag']) ? $attr['heading_tag'] : 'h3';
		$output .= '<'.$title_tag.' class="title heading">'.__( 'Related products', 'woocommerce' ).'</'.$title_tag.'>';
		$output  .= '<ul class="products '.$ul_classes.'">';
...
if (! function_exists('sc_product_additional_information')) {
	function sc_product_additional_information($attr, $product = false)
	{
		...
		if( ! empty( $attr['title'] ) ){
			$title_tag = !empty($attr['title_tag']) ? $attr['title_tag'] : 'h3';
			echo '<'.$title_tag.' class="title">'. __( 'Additional information', 'woocommerce' ) .'</'.$title_tag.'>';
		}
		...
		$output = '<div class="article_box mcb_column_wrapper">';
			...
			$output .= '<div class="desc_wrapper">';
				...
				if ($title) {
					...
					$output .= '<'.$title_tag.' class="title">'. wp_kses($title, mfn_allowed_html()) .'</'.$title_tag.'>';
				}
...
if (! function_exists('sc_helper')) {
	function sc_helper($attr, $content = null)
	{
		...
		if( !empty( $attr['link_type'] ) && !empty($attr['popup_id']) ){
			...
			$target_1_escaped = ' data-mfnpopup="'.$attr['popup_id'].'"';
		}

		if( !empty( $attr['link_type_2'] ) && !empty($attr['popup_id_2']) ){
			if( empty($link2) ) $link2 = '#';
			$class2 .= ' open-mfn-popup';
			$target_2_escaped = ' data-mfnpopup="'.$attr['popup_id_2'].'"';
		}
...
if (! function_exists('sc_feature_box')) {
	function sc_feature_box($attr, $content = null)
	{
		...
		if( !empty( $attr['link_type'] ) && !empty($attr['popup_id']) ){
			...
			$target_escaped .= ' data-mfnpopup="'.$attr['popup_id'].'"';
		}
		...
		$output = '<div class="story_box '. esc_attr($style) .'">';
				...
				if ( $link ) {
					...
					$output .= '<div class="desc_wrapper">';
						...
						if ($title) {
							$title_tag = !empty( $attr['title_tag'] ) ? $attr['title_tag'] : 'h3';
							$output .= '<'.$title_tag.' class="themecolor title">'. wp_kses($title, mfn_allowed_html()) .'</'.$title_tag.'>';
				...
				while ($query_blog->have_posts()) {
					...
					$output .= '<li class="'. esc_attr(implode(' ', get_post_class())) .'">';
						...
						$output .= '<div class="desc">';
							...
							if ($first) {
								$output .= '<'.$featured_title_tag.'><a href="'. esc_url(get_permalink()) .'">'. wp_kses(get_the_title(), mfn_allowed_html()) .'</a></'.$featured_title_tag.'>';
							} else {
								$output .= '<'.$items_title_tag.'><a href="'. esc_url(get_permalink()) .'">'. wp_kses(get_the_title(), mfn_allowed_html()) .'</a></'.$items_title_tag.'>';
							}
	...
	function sc_blog_teaser($attr, $content = null)
	{
		...
		$output = '<div class="blog-teaser '. esc_attr($class) .'">';
			...
			if ($title) {
				...
				$output .= '<'.$heading_tag.' class="title">'. wp_kses($title, mfn_allowed_html()) .'</'.$heading_tag.'>';
			}
						...
						$output .= '<div class="desc-wrapper">';
							$output .= '<div class="desc">';
								...
								$output .= '<div class="post-title">';
									$output .= '<'.$title_tag.'><a href="'. esc_url(get_permalink()) .'">'. wp_kses(get_the_title(), mfn_allowed_html()) .'</a></'.$title_tag.'>';
								$output .= '</div>';
									...
									$output .= '<a href="'. esc_url(get_the_permalink()) .'">';
										$output .= '<div class="hover_box_wrapper">';
											...
											if ($attachment_ids = $product->get_gallery_image_ids()) {
												...
												$output .= wp_get_attachment_image($secondary_image_id, 'woocommerce_thumbnail', '', $attr = array( 'class' => 'hidden_photo scale-with-grid' ));
											}
...
if (! function_exists('sc_call_to_action')) {
	function sc_call_to_action($attr, $content = null)
	{
		...
		if( !empty( $attr['link_type'] ) && !empty($attr['popup_id']) ){
			...
			$rel_escaped = 'data-mfnpopup="'.$attr['popup_id'].'"';
		}
...
if (! function_exists('sc_chart')) {
	function sc_chart($attr, $content = null)
	{
		...
		$output = '<div class="chart_box">';
			...
			$output .= '<'.$title_tag.'><big class="chart-desc">'. esc_html($title) .'</big></'.$title_tag.'>';
				...
				$output .= '<div class="mcb-column-inner">';
					$output .= '<div class="quick_fact">';
						$output .= '<div data-anim-type="zoomIn" class="animate zoomIn">';
							...
							$output .= '</div>';
							$output .= '<'.$title_tag.' class="title">'. esc_html($translate['days']) .'</'.$title_tag.'>';
					...
					$output .= '<div class="mcb-column-inner">';
						$output .= '<div class="quick_fact">';
							$output .= '<div data-anim-type="zoomIn" class="animate zoomIn">';
								...
								$output .= '</div>';
								$output .= '<'.$title_tag.' class="title">'. esc_html($translate['hours']) .'</'.$title_tag.'>';
								$output .= '<hr class="hr_narrow">';
							$output .= '</div>';
						$output .= '</div>';
					$output .= '</div>';
					...
					$output .= '<div class="mcb-column-inner">';
						$output .= '<div class="quick_fact">';
							$output .= '<div data-anim-type="zoomIn" class="animate zoomIn">';
								...
								$output .= '</div>';
								$output .= '<'.$title_tag.' class="title">'. esc_html($translate['minutes']) .'</'.$title_tag.'>';
								$output .= '<hr class="hr_narrow">';
							$output .= '</div>';
						$output .= '</div>';
					$output .= '</div>';
					...
					$output .= '<div class="mcb-column-inner">';
						$output .= '<div class="quick_fact">';
							$output .= '<div data-anim-type="zoomIn" class="animate zoomIn">';
								...
								$output .= '</div>';
								$output .= '<'.$title_tag.' class="title">'. esc_html($translate['seconds']) .'</'.$title_tag.'>';
								$output .= '<hr class="hr_narrow">';
							$output .= '</div>';
						$output .= '</div>';
					$output .= '</div>';
...
if (! function_exists('sc_image')) {
	function sc_image($attr, $content = null) {
		...
		if( !empty( $attr['link_type'] ) && !empty($attr['popup_id']) ){
			...
			$link_attr .= 'data-mfnpopup="'.$attr['popup_id'].'"';
		}
				...
				$output .= '<div class="image_wrapper">';
					...
					$output .= '<div class="image_links">';
						if ($link_image) {
							...
							$output .= '<a href="'. esc_url($link_image) .'" class="zoom" rel="prettyphoto" tabindex="-1" title="'. esc_attr($link_title ?? '') .'"><svg viewBox="0 0 26 26" aria-label="'. __('zoom image', 'betheme') .'"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><circle cx="11.35" cy="11.35" r="6" class="path"/><line x1="15.59" y1="15.59" x2="20.65" y2="20.65" class="path"/></svg></a>';
						}
						if ($link) {
							// This variable has been safely escaped above in this function
							$output .= '<a href="'. esc_url(be_dynamic_data($link)) .'" class="link '.$link_classes.'" '.$link_attr.' '. $target_escaped .' '. $onclick_escaped .' tabindex="-1" title="'. esc_attr($link_title ?? '') .'"><svg viewBox="0 0 26 26" aria-label="'. __('go to link', 'betheme') .'"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><path d="M10.17,8.76l2.12-2.12a5,5,0,0,1,7.07,0h0a5,5,0,0,1,0,7.07l-2.12,2.12" class="path"></path><path d="M15.83,17.24l-2.12,2.12a5,5,0,0,1-7.07,0h0a5,5,0,0,1,0-7.07l2.12-2.12" class="path"></path><line x1="10.17" y1="15.83" x2="15.83" y2="10.17" class="path"></line></g></svg></a>';
						}
					$output .= '</div>';
		...
		$output = '<div class="blockquote">';
			...
			if ($author) {
				$output .= '<p class="author">';
					...
					if ($link) {
						// This variable has been safely escaped above in this function
						$output .= '<a href="'. esc_attr($link) .'" '. $target_escaped .' title="'. esc_attr($link_title ?? '') .'">'. esc_html($author) .'</a>';
					} else {
 	...
 	function sc_icon_box_2($attr, $content = null) {
		...
		if( !empty($image) && strpos($image, '#') !== false ){
 		...
 		if( !empty( $attr['link_type'] ) && !empty($attr['popup_id']) ){
			...
			$target .= ' data-mfnpopup="'.$attr['popup_id'].'"';
		}
 			...
 			$output .= '<div class="'. esc_attr($classes) .'">';
 				...
 				$output .= '<div class="desc-wrapper">';
 					...
 					if( $title || !empty($is_love_item) ){
 						$output .= '<'. esc_attr($title_tag) .' class="title label">'. $title .'</'. esc_attr($title_tag) .'>';
 					}
...
if (! function_exists('sc_our_team')) {
	function sc_our_team($attr, $content = null)
	{
		...
		$output = '<div class="team team_'. esc_attr($style) .'">';
			...
			if ($heading) {
				$heading_tag = !empty( $attr['heading_tag'] ) ? $attr['heading_tag'] : 'h4';
				$output .= '<'.$heading_tag.' class="title">'. wp_kses($heading, mfn_allowed_html()) .'</'.$heading_tag.'>';
			}
			...
			$output .= '<div class="desc_wrapper clearfix">';
				...
				if ($title) {
					$title_tag = !empty( $attr['title_tag'] ) ? $attr['title_tag'] : 'h4';
					$output .= '<'.$title_tag.' class="desc_wrappper_title">';
					...
					$output .= '<div class="portfolio-item">';
						...
						$output .= '<a class="portfolio-details" href="'. esc_url($link) .'" '. $target_escaped .'>';
							...
							$output .= '<div class="details">';
								$output .= '<'.$title_tag.' class="title">'. get_the_title() .'</'.$title_tag.'>';
								if ($categories) {
					...
					while ($query->have_posts()) {
						...
						$output .= '<li class="content_slider_li_'. esc_attr($i) .'">';
							...
							if ($link) {
								...
								if ($style == 'carousel') {
									$title_tag = !empty( $attr['title_tag'] ) ? $attr['title_tag'] : 'p';
									$output .= '<'.$title_tag.' class="title">'. esc_html(get_the_title(get_the_ID())) .'</'.$title_tag.'>';
								}

								if ($style == 'description') {
									$title_tag = !empty( $attr['title_tag'] ) ? $attr['title_tag'] : 'h3';
									$output .= '<'.$title_tag.' class="title">'. get_the_title(get_the_ID()) .'</'.$title_tag.'>';
...
if (! function_exists('sc_accordion')) {
	function sc_accordion($attr, $content = null)
	{
		...
		$output = '<div class="accordion">';
			...
			if ($title) {
				$title_tag = !empty($attr['title_tag']) ? $attr['title_tag'] : 'h4';
				$output .= '<'.$title_tag.' class="title heading">'. wp_kses($title, mfn_allowed_html()) .'</'.$title_tag.'>';
			}
...
if (! function_exists('sc_faq')) {
	function sc_faq($attr, $content = null)
	{
		...
		$output = '<div class="faq">';
			...
			if ($title) {
				$title_tag = !empty( $attr['title_tag'] ) ? $attr['title_tag'] : 'h4';
				$output .= '<'.$title_tag.' class="heading">'. wp_kses($title, mfn_allowed_html()) .'</'.$title_tag.'>';
			}
