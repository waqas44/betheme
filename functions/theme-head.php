...
if (! function_exists('mfn_seo')) {
	function mfn_seo()
	{
		...
		if ( mfn_opts_get('google-gtag-js') ) {
			echo "\n";
...
if (! function_exists('mfn_seo_body')) {
	function mfn_seo_body(){
		...
		if ( mfn_opts_get('google-gtag-html') ) {
			echo "\n";
 	...
 	if( 'inline' == mfn_opts_get('local-styles-location') && empty( $_GET['visual'] ) ) {
 		...
 		if( is_singular() ) {
 			...
 			if( function_exists('is_woocommerce') && is_singular('product') && !empty($mfn_global['single_product']) && get_post_status($mfn_global['single_product']) == 'publish' ){
 				...
 				$id = $mfn_global['blog'];
 			}else if( ( is_tax() || ( is_page() && get_the_ID() == mfn_opts_get('portfolio-page') ) ) && !empty($mfn_global['portfolio']) && get_post_status($mfn_global['portfolio']) == 'publish' ){
...
if (! function_exists('mfn_bebuilder_access')) {
	function mfn_bebuilder_access( $return ) {
		...
		if( !empty(mfn_opts_get('builder-visibility')) && mfn_opts_get('builder-visibility') == 'hide' ) {
			return false;
		}
...
function mfn_visual_builder(){
	...
	if( apply_filters('bebuilder_preview', false) ) {
		...
		function mfnvb_body_classes( $classes ) {
		    ...
		    if( $post_type == 'template' && !empty(get_post_meta($post_id, 'mfn_template_type', true)) ){
		    	$classes[] = 'mfn-bebuilder-'.get_post_meta($post_id, 'mfn_template_type', true);
		    }
